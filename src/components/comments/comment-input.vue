...
<template>
  <div v-show="show" ref="inputEl">
    <div class="mb-2 flex gap-x-2 items-center">
      <el-input v-model="text" :placeholder="selected ? `回复 ${selected.authorId.nickname}` : '评论'" @focus="focused = true" @blur="focused = false" />
      <el-icon class="cursor-pointer" size="24" @click="tapEmoji">
        <Emoji16Regular />
      </el-icon>
    </div>
    <div v-if="showEmoji" class="flex flex-wrap gap-x-0.5">
      <el-icon v-for="(item, index) in [...EMOJI, ...EMOJI, ...EMOJI, ...EMOJI]" :key="index" class="cursor-pointer" size="24">
