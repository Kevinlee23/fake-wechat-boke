  ...
  <div class="px-[20px] pt-[20px]">
    <div class="flex mb-[16px]">
      ...
      <div class="flex-1 flex flex-col gap-y-[4px] ml-[12px]">
        ...
        <div v-if="model.address" class="text-[12px] text-[#576b95]">
          {{ model.address }}
        </div>

        <div class="flex justify-between">
          <div class="group/item">
            <div class="text-[12px] text-[#9ca3af] hidden group-hover/item:block">
              {{ model.createTime }}
            </div>
            <div class="text-[12px] text-[#9ca3af] group-hover/item:hidden">
          ...
          <el-dropdown class="boke-drop" trigger="click" placement="left" :teleported="false" @command="handleEvent">
            ...
            <template #dropdown>
              <el-dropdown-menu>
                <el-dropdown-item>
                  <el-icon class="text-[18px] mr-[4px]">
                    <StarFilled />
                  </el-icon>
                  点赞
                </el-dropdown-item>
        ...
        <div class="px-[8px]bg-[#f7f7f7]" :class="{ 'py-[4px]': model.likeNum > 0 || comments.length > 0 }">
          ...
          <div v-for="(item, index) in comments" :key="item._id" class="flex" :class="{ 'mb-[4px]': index !== comments.length - 1 }">
            ...
            <div class="ml-[8px] cursor-pointer" @click="tapInput(item)">
              {{ item.content }}
            </div>
          </div>
        </div>
